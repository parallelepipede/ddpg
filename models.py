...
class Actor:
    ...
    def build_network(self,n_states,n_actions,action_high):
        ...
        x = Dense(600,activation = tf.nn.relu,kernel_initializer = self.initializer, name = "actor_f1")(inputs)  #kernel_initializer = self.initializer
...
class Critic:
    ...
    def build_network(self,n_states,n_actions,action_high):
        ...
        x = Dense(600,activation = tf.nn.relu,kernel_initializer = self.initializer, name = "critic_f1")(state_inputs)
...
class Agent(object):
    def __init__(self, n_states, n_actions, action_high, action_low, gamma=.99, rho=.001,std_dev=.2):
        ...
        self.actor_target = Actor().build_network(n_states,n_actions,action_high)
    ...
    def update_model(self,observation):
        state,action,reward,next_state = observation
        
        print()
        print("update_model", observation)
        print()
        with tf.GradientTape() as gradientTape:
            gradientTape.watch(tf.Variable(next_state))
            print("next state", next_state)
            print()
            print("next state tensorflow", tf.convert_to_tensor(next_state))
            self.actor_target(tf.convert_to_tensor(next_state))
        ...
        critic_grad = gradientTape.gradients(L, self.critic.trainable_variables)
        ...
        actor_grad = gradientTape.gradients(action_loss,self.actor.trainable_vairables)
        self.actor.optimizer.apply_gradients(zip(actor_grad,self.actor.trainable_variables))
        
        return L, action_loss
      
    def update_target_network(self,model_target,model_ref,rho):
    ...
    def learn(self):
        ...
        critic_loss,action_loss = self.update_model(self.replay_buffer.replay_buffer[-1])
        self.update_target_network(self.actor_target,self.actor,self.rho)
        self.update_target_network(self.critic_target,self.critic,self.rho)
        
        return critic_loss, action_loss
